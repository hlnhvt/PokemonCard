...
export async function recognizeCardWithOCR(imageSource, onProgress) {
  try {
    ...
    // Ignore common TCG card stop words that are NOT Pokemon names
    ...
    const candidateTokens = cleanTokens.filter((w) => !STOP_WORDS.has(w) && !/^\d+$/.test(w));
    const allNames = await getAllPokemonNames();

    // Score all official Pokemon names against extracted candidate tokens
    const scoredList = [];
    ...
    for (const name of allNames) {
      ...
      for (const token of candidateTokens) {
        ...
        if (token.includes(upperName) || (upperName.length >= 5 && upperName.includes(token))) {
          ...
          bestScore = Math.max(bestScore, Math.round(ratio * 90));
        }

        // Bigram similarity
        const sim = stringSimilarity(token, upperName);
        if (sim >= 0.45) {
          bestScore = Math.max(bestScore, Math.round(sim * 95));
        }
      }
    ...
    const topCandidates = scoredList.slice(0, 4);
