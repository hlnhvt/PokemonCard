...
import { DifficultyPicker } from '../team/DifficultyPicker';
import { readTeamDifficulty } from '../team/difficultyChoice';
...
function Road({ state, team, lead, onLead, difficulty, onDifficulty, onChallenge, onChangeTeam }) {
  return (
...
export function LeagueGame({ collection = [], allowScanned = false, onScanned, onGold, onClose, random = Math.random }) {
  ...
  const challenge = async () => {
    ...
    setError(null);
    try {
      const s = await loadTeamBattle({ team: withLead(team, lead), opponentNames: gym.team, arena, levelFactor: gym.level, difficulty, random, onProgress: () => alive.current && setProgress((p) => p + 1) });
  ...
  const onFinish = ({ won }) => {
    ...
    const gold = goldForGym(index, difficulty);
    onGold?.(gold);
  ...
  return createPortal(
    <div data-theme="dark" className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-0 sm:p-4 select-none" role="dialog" aria-label="Giải đấu Liên minh" data-screen={screen} data-index={state.index} style={{ '--battle-tempo': TEMPO[speed] }}>
      <div className="relative flex flex-col w-full h-full sm:h-auto sm:max-h-full max-w-md overflow-y-auto overflow-x-hidden sm:rounded-3xl sm:border-4 border-white/70 shadow-2xl bg-gradient-to-b from-indigo-900 via-purple-900 to-slate-950">
        ...
        {screen === 'road' && <Road state={state} team={team} lead={lead} onLead={setLead} difficulty={difficulty} onDifficulty={setDifficulty} onChallenge={challenge} onChangeTeam={() => setScreen('build')} />}
